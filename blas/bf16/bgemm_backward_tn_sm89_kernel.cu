// Bgemm backward TN: C = alpha * A^T * B + beta * C (BF16, dB = dX^T * dY).
    ...
    int batchCount)
{
    ...
    auto load_tile = [&](int stage, int ko) {
        ...
        for (int i = 0; i < 4; i++) {
            int r = (tid / 16) + i * 8; 
            int c = (tid % 16) * 8;     
            const __nv_bfloat16* g_ptr = A_batch + (long long)(ko + r) * lda + (by * BM + c);
            int swizzled_c = c ^ ((r & 7) << 3); 
            uint32_t sm_addr = __cvta_generic_to_shared(&s_mem[stage * STG_SZ + r * BM + swizzled_c]);
            int bytes = (ko + r < K && by * BM + c < M) ? max(0, min(16, (M - (by * BM + c)) * 2)) : 0;
        ...
        for (int i = 0; i < 4; i++) {
            int r = (tid / 16) + i * 8; 
            int c = (tid % 16) * 8;     
            const __nv_bfloat16* g_ptr = B_batch + (long long)(ko + r) * ldb + (bx * BN + c);
            int swizzled_c = c ^ ((r & 7) << 3); 
            uint32_t sm_addr = __cvta_generic_to_shared(&s_mem[stage * STG_SZ + AS_SIZE + r * BN + swizzled_c]);
            int bytes = (ko + r < K && bx * BN + c < N) ? max(0, min(16, (N - (bx * BN + c)) * 2)) : 0;
            asm volatile("cp.async.cg.shared.global [%0], [%1], 16, %2;\n" :: "r"(sm_addr), "l"(g_ptr), "r"(bytes));
        }
    };
    ...
    auto load_regA = [&](uint32_t reg[4], int ki, int mi, int st) {
        int k = ki + (lane % 8) + (lane / 16) * 8;
        int m = wy * 64 + mi * 16 + (lane / 8 % 2) * 8;
        int swizzled_m = m ^ ((k & 7) << 3);
        uint32_t addr = __cvta_generic_to_shared(&s_mem[st * STG_SZ + k * BM + swizzled_m]);
        asm volatile("ldmatrix.sync.aligned.m8n8.x4.trans.shared.b16 {%0, %1, %2, %3}, [%4];" 
                     : "=r"(reg[0]), "=r"(reg[1]), "=r"(reg[2]), "=r"(reg[3]) : "r"(addr));
    };

    auto load_regB = [&](uint32_t reg[2], int ki, int ni, int st) {
        int k = ki + (lane % 8) + (lane / 8 % 2) * 8;
        int n = wx * 64 + ni * 8;
        int swizzled_n = n ^ ((k & 7) << 3);
        uint32_t addr = __cvta_generic_to_shared(&s_mem[st * STG_SZ + AS_SIZE + k * BN + swizzled_n]);
...
template __global__ void bgemm_tn_backward_kernel<true>(int,int,int,__nv_bfloat16,const __nv_bfloat16*,int,long long int,const __nv_bfloat16*,int,long long int,__nv_bfloat16,__nv_bfloat16*,int,long long int,int);
template __global__ void bgemm_tn_backward_kernel<false>(int,int,int,__nv_bfloat16,const __nv_bfloat16*,int,long long int,const __nv_bfloat16*,int,long long int,__nv_bfloat16,__nv_bfloat16*,int,long long int,int);
