// Batched BF16 GEMM, 128x128x64 tile, 3-stage cp.async pipeline, bf16 MMA.
    ...
    int batchCount)
{
    ...
    auto load_to_stage = [&](int stage, int k_off) {
        // Load A: 128 (M) x 64 (K). 128 threads.
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            // Coalesced loading: 8 threads per row.
            int r = (tid / 8) + i * 16;
            int c = (tid % 8) * 8;
        ...
        // Load B: 64 (K) x 128 (N). 128 threads.
        #pragma unroll
        for (int i = 0; i < 8; i++) {
...
template __global__ void bgemm_NN_128x128_64x3_kernel<MYCUBLAS_OP_N, MYCUBLAS_OP_N>(int,int,int,__nv_bfloat16,const __nv_bfloat16*,int,long long int,const __nv_bfloat16*,int,long long int,__nv_bfloat16,__nv_bfloat16*,int,long long int,int);
template __global__ void bgemm_NN_128x128_64x3_kernel<MYCUBLAS_OP_N, MYCUBLAS_OP_T>(int,int,int,__nv_bfloat16,const __nv_bfloat16*,int,long long int,const __nv_bfloat16*,int,long long int,__nv_bfloat16,__nv_bfloat16*,int,long long int,int);
template __global__ void bgemm_NN_128x128_64x3_kernel<MYCUBLAS_OP_T, MYCUBLAS_OP_N>(int,int,int,__nv_bfloat16,const __nv_bfloat16*,int,long long int,const __nv_bfloat16*,int,long long int,__nv_bfloat16,__nv_bfloat16*,int,long long int,int);
template __global__ void bgemm_NN_128x128_64x3_kernel<MYCUBLAS_OP_T, MYCUBLAS_OP_T>(int,int,int,__nv_bfloat16,const __nv_bfloat16*,int,long long int,const __nv_bfloat16*,int,long long int,__nv_bfloat16,__nv_bfloat16*,int,long long int,int);
