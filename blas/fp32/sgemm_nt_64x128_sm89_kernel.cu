// Batched SGEMM, NT layout, 64x128 tile, TF32 tensor-core MMA with cp.async staging.

#include <cuda_runtime.h>
#include <stdint.h>

// 4x ldmatrix from shared memory into 4 registers.
#ifndef LDSM_X4
...
namespace nt64x128 {
    static constexpr int BM = 64, BN = 128, BK = 16, STAGES = 3, THREADS = 128;
    static constexpr int AS_SIZE = BM * BK, BS_SIZE = BN * BK, STAGE_SIZE = AS_SIZE + BS_SIZE;
}

// m16n8k8 TF32 MMA accumulating into d0..d3.
...
sgemm_nt_64x128_sm89_2_k(int M, int N, int K, float alpha, const float* A, int lda, long long strideA, const float* B, int ldb, long long strideB, float beta, float* C, int ldc, long long strideC, int batchCount) {
    using namespace nt64x128;
    ...
    const int wy = wid % 2, wx = wid / 2; // 2x2 grid. Warp Tile: M=32, N=64.
    ...
    for (int kt=0; kt<k_tiles; kt++) {
        ...
        for (int k_step = 0; k_step < 2; k_step++) {
            ...
            for (int mi=0; mi<2; mi++) loadA(frA[k_step%2][mi], ks, mi, smem+rs*STAGE_SIZE);
            for (int ni=0; ni<8; ni++) loadB(frB[k_step%2][ni], ks, ni, smem+rs*STAGE_SIZE+AS_SIZE);

            #pragma unroll
            for (int mi=0; mi<2; mi++) {
                for (int ni=0; ni<8; ni++) {
                    mma_tf32(acc[mi][ni][0], acc[mi][ni][1], acc[mi][ni][2], acc[mi][ni][3],
                             frA[k_step%2][mi][0], frA[k_step%2][mi][1], frA[k_step%2][mi][2], frA[k_step%2][mi][3],
                             frB[k_step%2][ni][0], frB[k_step%2][ni][1]);
                }
            }
        }
    ...
    #pragma unroll
    for (int mi=0; mi<2; mi++) {
        const int r0 = by*BM+wy*32+mi*16+g_epi, r8 = r0+8;
        if (r0 >= M) continue;
        #pragma unroll
        for (int ni=0; ni<8; ni++) {
